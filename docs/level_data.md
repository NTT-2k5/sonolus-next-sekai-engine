...
* **maskNotes (bool)**: When true, note edges are clamped to the stage mask in play and watch modes. Notes fully outside the mask are hidden. Connectors using the default presentation retain their original shape and are clipped only when masking is enabled for both their head and tail; masks from different stages are interpolated along the connector. Hitboxes and preview rendering are unaffected. Defaults to false.
* **ease (EaseType)**
